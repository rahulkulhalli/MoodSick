<template>
  <div>
    Spotify Parameter Aggregates by Mood
    <div id="spotifyParamaHistory" style="height: 80vh; width: 100%"></div>
  </div>
</template>
...
export default {
  ...
  mounted() {
    this.$nextTick(() => {
      var app = {};
      var chartDom = document.getElementById("spotifyParamaHistory");
      ...
      for (const key of legendData) {
        ...
        for (let index = 0; index < xAxisKeys.length; index++) {
          ...
          //  temp_arr.push(Number(this.graphData[element][key].toString(5)));
        }
        seriesData.push({
          name: key,
          type: "bar",
